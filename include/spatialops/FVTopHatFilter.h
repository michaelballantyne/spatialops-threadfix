...
namespace SpatialOps{
  ...
  // forward declaration.
  namespace FVStaggered{ template<typename T1> class TopHatFilterAssembler; }
  ...
  struct OpAssemblerSelector< FVStaggered::Filter, FieldT, FieldT >
  {
    typedef FVStaggered::TopHatFilterAssembler<FieldT>  Assembler;
  };
  ...
  namespace FVStaggered{
    ...
    private:
      ...
      void get_npts( const IndexTriplet& loc, int& ntot, std::vector<int>& nxyz ) const;
    ...
    get_ncols() const
    {
      return get_n_tot<FieldT>(dim_,hasPlusXSideFaces_,hasPlusYSideFaces_,hasPlusZSideFaces_);
    }
    ...
    get_row_entries( const int irow,
                     ...
                     std::vector<int> & ixs ) const
    {
      const IndexTriplet ijk( flat2ijk<FieldT>::value( dim_, irow, hasPlusXSideFaces_, hasPlusYSideFaces_, hasPlusZSideFaces_ ) );
      ...
      for( int ipts=-nside[0]; ipts<=nside[0]; ++ipts ){
        for( int jpts=-nside[1]; jpts<=nside[1]; ++jpts ){
          for( int kpts=-nside[2]; kpts<=nside[2]; ++kpts ){
            const IndexTriplet ixt( ijk.i+ipts, ijk.j+jpts, ijk.k+kpts );
    ...
    void
    TopHatFilterAssembler<FieldT>::
    get_npts( const IndexTriplet& loc, int& ntot, std::vector<int>& nxyz ) const
    {
      ntot = 1;
      if( dim_[0] > 1 ){
        const int nx = get_nx<FieldT>(dim_,hasPlusXSideFaces_);
      ...
      if( dim_[1] > 1 ){
        const int ny = get_ny<FieldT>(dim_,hasPlusYSideFaces_);
      ...
      if( dim_[2] > 1 ){
        const int nz = get_nz<FieldT>(dim_,hasPlusZSideFaces_);
