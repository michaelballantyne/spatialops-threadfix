...
inline void evaluate_serial_example(FieldType & result,
    ...
    RUN_TEST(// X - direction
	     ...
	     // Y - direction
             tmpPhi <<= phi + cos(field1);
             tmpDCoef <<= dCoef + cos(field2);
	     ...
	     // Z - direction
             tmpPhi <<= phi + tan(field1);
             tmpDCoef <<= dCoef + tan(field2);
...
inline void evaluate_chaining_example(FieldType & result,
    ...
    RUN_TEST(result <<= (- neboDivX(neboGradX(phi + sin(field1)) * neboInterpX(dCoef + sin(field2)))
                         - neboDivY(neboGradY(phi + cos(field1)) * neboInterpY(dCoef + cos(field2)))
                         - neboDivZ(neboGradZ(phi + tan(field1)) * neboInterpZ(dCoef + tan(field2)))),
             "new");
